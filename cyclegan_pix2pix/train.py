...
if __name__ == "__main__":
    ...
    if not osp.exists(opt.checkpoints_dir):
        os.makedirs(opt.checkpoints_dir)

    logfile = open(osp.join(opt.checkpoints_dir, "log.txt"), "w")
    ...
    opt.epoch_size = dataset_size // opt.batch_size

    if "cycle_gan" == opt.model:
        print(
            "multiplying epoch_size for quantize/pruning with 3 to adjust for cyclegan training"
        )
        opt.epoch_size *= 3
