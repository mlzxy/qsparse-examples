    ...
    def p(*args):
        if "prune" in train_mode:
            ...
            if "weight" in train_mode:
                return (
                    nn.Identity() if len(args) == 0 else prune(args[0], **kw, window_size=1)
                )
            elif "feat" in train_mode:
                return prune(**kw, window_size=ws, strict=False) if len(args) == 0 else args[0]
            elif "both" in train_mode:
                return prune(**kw, window_size=ws, strict=False) if len(args) == 0 else prune(args[0], **kw, window_size=1)
        else:
            return bypass(*args)
