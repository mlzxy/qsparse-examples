...
from test_net import validation
...
if __name__ == "__main__":
    ...
    cfg.ROOT_DIR = osp.join(args.save_dir, "output")
    print(f"cfg.DATA_DIR = {cfg.DATA_DIR}")

    imdb, roidb, ratio_list, ratio_index = combined_roidb(args.imdb_name)
    train_size = len(roidb)

    imdb_val, roidb_val, ratio_val_list, ratio_val_index = combined_roidb(
    ...
    print(f"epoch size = {len(dataloader)}")
    ...
    if args.resume:
        load_name = os.path.join(output_dir, "faster_rcnn_*.pth".format())
        pths = sorted(glob(load_name))
        if len(pths) > 0:
            load_name = pths[-1]
            ...
            try:
                for _k in ["qin.decimal", "qin._n_updates", "qin.bits"]:
                    if _k in checkpoint["model"]:
                        checkpoint["model"].pop(_k)

                fasterRCNN.load_state_dict(checkpoint["model"])
                for k, v in checkpoint["model"].items():
                    if k.endswith("updates"):
                        print(k, v.item())
    ...
    for epoch in range(args.start_epoch, args.max_epochs + 1):
        ...
        _state_dict = (
            fasterRCNN.module.state_dict() if args.mGPUs else fasterRCNN.state_dict()
        )
        ...
        if epoch >= 5:
            ...
            validation(
                fasterRCNN,
                (imdb_val, roidb_val, ratio_val_list, ratio_val_index),
                cfg,
                vis=False,
                args=args,
                cuda=args.cuda,
