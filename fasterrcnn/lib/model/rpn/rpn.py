...
class _RPN(nn.Module):
    ...
    def __init__(self, din, train_mode, epoch_size):
        ...
        self.RPN_Conv = q(nn.Conv2d(self.din, 512, 3, 1, 1, bias=True))
        self.q1 = q()
        ...
        self.RPN_cls_score = q(nn.Conv2d(512, self.nc_score_out, 1, 1, 0))

        # define anchor box offset prediction layer
        self.nc_bbox_out = (
            len(self.anchor_scales) * len(self.anchor_ratios) * 4
        )  # 4(coords) * 9 (anchors)
        self.RPN_bbox_pred = q(nn.Conv2d(512, self.nc_bbox_out, 1, 1, 0))
    ...
    def forward(self, base_feat, im_info, gt_boxes, num_boxes):
        ...
        rpn_conv1 = self.q1(F.relu(self.RPN_Conv(base_feat), inplace=True))
