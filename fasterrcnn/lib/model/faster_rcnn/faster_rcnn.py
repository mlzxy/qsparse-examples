...
class _fasterRCNN(nn.Module):
    ...
    def __init__(self, classes, class_agnostic, train_mode, epoch_size):
        ...
        # define rpn
        self.RCNN_rpn = _RPN(self.dout_base_model, train_mode, epoch_size)
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes):
        ...
        # im_data = self.qin(im_data)
        # We skip to quantize the input, because vanilla faster rcnn uses
        # a skewed pixel mean as in (../utils/config.py), which can be harmful to quantization.
        # Moreover, image is naturally quantized. So it's straightforward to train with
        # input normalized by (x - 128) / 128.
        ...
        pooled_feat = self.qpool(pooled_feat)
